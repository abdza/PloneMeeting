...
def setupplonemeeting_onestate_workflow(self, workflow):
    """Define the plonemeeting_onestate_workflow workflow.
    """
    # Add additional roles to portal
    portal = getToolByName(self,'portal_url').getPortalObject()
    data = list(portal.__ac_roles__)
    for role in ['MeetingManager', 'MeetingObserverGlobal']:
        if not role in data:
            data.append(role)
                ...
                if prm is not None:
                    try:
                        prm.addRole(role, role, 
                                    "Added by product 'PloneMeeting'/workflow 'plonemeeting_onestate_workflow'")
    ...
    workflow.setProperties(title='plonemeeting_onestate_workflow')
    ...
    stateDef.setPermission(View,
                           0,
                           ['Owner', 'Anonymous', 'Manager', 'MeetingManager', 'MeetingObserverGlobal'])
...
def createplonemeeting_onestate_workflow(self, id):
    """Create the workflow for PloneMeeting.
    """

    ob = DCWorkflowDefinition(id)
    setupplonemeeting_onestate_workflow(self, ob)
    return ob

addWorkflowFactory(createplonemeeting_onestate_workflow,
                   id='plonemeeting_onestate_workflow',
